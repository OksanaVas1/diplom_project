...
def prepair_data(driver):
    ...
    auth_page.login_input.send_keys('157@fulwark.com')
    auth_page.password_input.send_keys('Test123')
...
def test_delete_patronymic(driver):
    ...
    # Добавляем отчество
    page_id_authorized.edit_name_btn.click()
    page_id_authorized.fathers_name_input.send_keys('отчество')
    page_id_authorized.save_btn.is_clickable()
    page_id_authorized.save_btn.click()
...
def test_error_massage_if_invalid_pass(driver):
    ...
    page.login_input.send_keys('123@fulwark.com')
    page.password_input.send_keys('Test1209')
    page.enter_btn.click()
...
def test_error_massage_if_empty_telephone_in_recovery_pass(driver):
    """Проверка что если попытаться "Продолжить" восстановление пароля не вводя телефон,
...
def test_error_massage_if_empty_email_in_recovery_pass(driver):
    """Проверка что если попытаться "Продолжить" восстановление пароля не вводя email,
    выводится сообщение об ошибке"""
...
def test_error_message_if_empty_captcha_in_recovery_pass(driver):
    """Проверка что если попытаться "Продолжить" восстановления пароля с валидным email не вводя капчу,
    ...
    auth_page.forgot_password_btn.click()
    recovery_page.email_btn.click()
    recovery_page.login_input.send_keys('3287@fulwark.com')
    recovery_page.next_btn.click()
